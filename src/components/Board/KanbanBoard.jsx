...
export default function KanbanBoard({ columns, jobs, onMoveJob, onEditJob, isFiltered = false, filteredEmptyState }) {
  ...
  return (
    <div className="modal-scrollbar -mx-4 flex [touch-action:pan-x_pan-y] snap-x snap-mandatory items-start gap-4 overflow-x-auto px-4 pb-3 [-webkit-overflow-scrolling:touch] sm:-mx-6 sm:gap-5 sm:px-6 xl:mx-0 xl:grid xl:grid-cols-5 xl:gap-6 xl:overflow-visible xl:px-0 xl:pb-0">
      {columns.map((column) => (
        <Column
          key={column.id}
          column={column}
